...
describe("formatTime", () => {
  it("残り時間を HH:MM:SS 形式で表示する", () => {
    ...
    expect(formatTime(THIRTY_MINUTES_MS)).toBe("00:30:00");
...
describe("useTimer", () => {
  ...
  it("8時間からカウントダウンし progress が減少する", () => {
    ...
    expect(result.current.progress).toBeCloseTo(
      (EIGHT_HOURS_MS - 1000) / EIGHT_HOURS_MS,
    );
  });
  ...
  it("autoStart: false では reset で 30 分タイマーを開始できる", () => {
    const { result } = renderHook(() =>
      useTimer({ totalMs: THIRTY_MINUTES_MS, autoStart: false }),
    );

    expect(result.current.formattedTime).toBe("00:30:00");
    expect(result.current.isRunning).toBe(false);
    ...
    expect(result.current.formattedTime).toBe("00:29:59");
    expect(result.current.progress).toBeCloseTo(
      (THIRTY_MINUTES_MS - 1000) / THIRTY_MINUTES_MS,
    );
  });
  ...
  it("0 秒通過時に onNaturalZeroCross を呼ぶ", () => {
    ...
    renderHook(() => useTimer({ totalMs: 2_000, onNaturalZeroCross }));
  ...
  it("指定したしきい値を自然に通過したとき milestones の onCross を呼ぶ", () => {
    const onCross = vi.fn();

    renderHook(() =>
      useTimer({
        totalMs: 5_000,
        milestones: [{ atRemainingMs: 3_000, onCross }],
      }),
    );
  ...
  it("長時間の復帰でしきい値を通過済みのとき milestones の onCross を呼ばない", () => {
    const onCross = vi.fn();
